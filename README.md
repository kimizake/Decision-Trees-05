...
To run cross validation, run ``python cross-validate.py NAME k`` where ``NAME`` 
is the literal name of the file in ``data/`` without a ``.txt`` extension,
and ``k`` is an integer value for the number of folds desired.

To prune a decision tree, run ``python prune.py NAME`` where ``NAME`` 
is the literal name of the file in ``data/`` without a ``.txt`` extension,
 i.e. to prune the DT generated from ``data/train_sub.txt`` we would run ``python prune.py train_sub``
